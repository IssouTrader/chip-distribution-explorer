...
HEADERS = {"User-Agent": "Recherche personnelle contact@example.com"}
...
def search_by_name(query, limit=8):
    ...
    les 14 tickers déjà couverts en détail, puis dans la base SEC complète
    (Nasdaq/NYSE), en repli."""
    ...
    with st.spinner(L["spinner"].format(ticker=ticker_input)):
        if ticker_input in CURATED:
            ...
            if not cik:
                ...
                st.stop()
            cps = get_sec_checkpoints(cik)
            if not cps:
                st.error(L["error_no_sec_data"].format(ticker=ticker_input))
                st.stop()
            cfg = {"start": cps[0][0], "bucket": None, "checkpoints": cps, "events": {}}
            ...
            try:
                currency = yf.Ticker(ticker_input).info.get("currency", "USD")
            except Exception:
                currency = "USD"
