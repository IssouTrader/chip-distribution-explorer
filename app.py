...
STRINGS = {
    "fr": {
        ...
        "news_none": "Aucune actualité récente disponible pour ce ticker.",
    },
    "en": {
        "app_title": "Chip Distribution Explorer",
        ...
        "news_none": "No recent news available for this ticker.",
    },
}
...
def get_recent_news(ticker, limit=5):
    """Récupère les derniers titres d'actualité via Yahoo Finance (gratuit,
    pas de clé API). Analyse défensive du format -- yfinance a changé la
    structure de cette donnée plusieurs fois par le passé."""
    items = []
    for attempt in range(3):
        try:
    ...
    for entry in raw[:limit]:
        content = entry.get("content", entry)  # ancien format vs nouveau format
        ...
        if isinstance(pub_date, (int, float)):
            try:
                date_str = ddate.fromtimestamp(pub_date).strftime("%Y-%m-%d")
            except Exception:
                pass
...
if ticker_input:
    ...
    # --- Actualités récentes : affichées en barre latérale, sous la langue ---
    with st.sidebar:
        st.markdown(
            f'<div style="font-family:\'JetBrains Mono\',monospace; font-size:11px; color:#7C6CF5; '
            f'letter-spacing:1px; margin-top:18px; margin-bottom:8px;">📰 {L["news_title"].upper()} — {ticker_input}</div>',
            unsafe_allow_html=True,
        )
        news_items = get_recent_news(ticker_input)
        if news_items:
            for item in news_items:
                meta = " · ".join(x for x in [item["publisher"], item["date"]] if x)
                title_html = (
                    f'<a href="{item["link"]}" target="_blank" style="color:#E8EAED; text-decoration:none;">{item["title"]}</a>'
                    if item["link"] else f'<span style="color:#E8EAED;">{item["title"]}</span>'
                )
                st.markdown(
                    f'<div style="background:#131F30; border:1px solid #22334A; border-radius:12px; '
                    f'padding:10px 12px; margin-bottom:8px;">'
                    f'<div style="font-size:13px; font-weight:600; line-height:1.35;">{title_html}</div>'
                    f'<div style="color:#9AA5B1; font-size:11px; margin-top:4px;">{meta}</div></div>',
                    unsafe_allow_html=True,
                )
        else:
            st.caption(L["news_none"])
